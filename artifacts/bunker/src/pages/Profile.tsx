...
import { User, Shield, Server, Eye, Bell, Camera, Skull, LogOut, Key } from "lucide-react";
  ...
  const handleClick = () => {
    if (phase === "idle") {
      ...
      const iv = setInterval(() => {
        c -= 1;
        setCount(c);
        if (c === 0) { clearInterval(iv); selfDestruct(); }
      }, 600);
    }
  };
  ...
  return (
    <div className="min-h-screen pb-28 px-4 pt-12 no-scrollbar overflow-y-auto">
      ...
      <div className="space-y-5">
        ...
        {/* ── Settings toggles (notifications + screenshots only) ── */}
