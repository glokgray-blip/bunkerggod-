...
import { Brain, Search, X, ShieldAlert, ShieldCheck, Shield, EyeOff, ArrowRight, Lock } from "lucide-react";
...
function StatusBar({ url }: { url: string }) {
  ...
  const trackers = 7;
  const riskColor = trackers > 5 ? "#ff3366" : trackers > 2 ? "#ffd700" : "#00ff88";

  return (
    <div className="flex items-center gap-3 px-3 py-1.5 overflow-x-auto no-scrollbar"
      ...
      <div className="flex items-center gap-1 px-2 py-0.5 rounded-sm shrink-0 font-tech text-[10px] uppercase tracking-widest"
        style={{ background: "#00ff8810", border: "1px solid #00ff8830", color: "#00ff88" }}>
        <Lock className="w-3 h-3" />
        <span>{t("browser.secure")}</span>
      </div>
      <div className="flex items-center gap-1 px-2 py-0.5 rounded-sm shrink-0 font-tech text-[10px] uppercase tracking-widest"
        style={{ background: `${riskColor}10`, border: `1px solid ${riskColor}30`, color: riskColor }}>
        <EyeOff className="w-3 h-3" />
        <span>{t("browser.trackers", { count: trackers })}</span>
      </div>
      <div className="ml-auto shrink-0 text-gray-600 font-mono text-[10px] truncate max-w-[120px]">
          ...
          <>
            ...
            <motion.div key="sheet"
              ...
              style={{ background: "rgba(5,5,10,0.97)", borderTop: "1px solid rgba(255,0,204,0.3)",
                boxShadow: "0 -20px 60px rgba(255,0,204,0.1)", backdropFilter: "blur(24px)" }}>
