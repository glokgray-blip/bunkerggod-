...
function MeshBar() {
  ...
  return (
    <motion.div
      ...
      style={{ background: "rgba(0,0,0,0.5)", border: `1px solid ${active ? "#00ff8830" : scanning ? "#ffd70030" : "rgba(255,255,255,0.06)"}`,
        backdropFilter: "blur(12px)", boxShadow: active ? `0 0 24px rgba(0,255,136,0.08)` : "none",
      ...
      <div className="flex items-center gap-4 p-4">
        ...
        <div className="flex-1 min-w-0 space-y-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-tech text-xs uppercase tracking-widest" style={{ color: statusColor, textShadow: active ? T.glowText(statusColor) : "none" }}>
              MESH
            </span>
            {/* Live indicator */}
            {active && (
          ...
          <AnimatePresence mode="wait">
            <motion.p
              ...
              style={{ color: active ? "#fff" : "#444", textShadow: active ? T.glowText("#00ff88") : "none" }}
            >
              {statusLabel}
            </motion.p>
          </AnimatePresence>

          {/* Signal bars for active state */}
          {active && (
      ...
      <AnimatePresence>
        {active && expanded && (
          <motion.div
            ...
            style={{ borderTop: "1px solid rgba(0,255,136,0.1)", overflow: "hidden" }}
          >
    ...
    <div className="min-h-screen pb-28 px-4 pt-12 no-scrollbar overflow-y-auto">
      ...
      <div className="mb-5 p-3 flex gap-3 items-start rounded-sm"
        style={{ background: "rgba(0,0,0,0.4)", border: "1px solid rgba(0,240,255,0.1)",
          borderLeft: "2px solid rgba(0,240,255,0.5)", backdropFilter: "blur(8px)" }}>
        <Shield className="w-4 h-4 shrink-0 mt-0.5" style={{ color: "#00f0ff" }} />
        <div>
          <p className="font-tech text-[10px] text-white uppercase tracking-wider mb-0.5">
            {t("chats.protocolTitle")}
          </p>
          <p className="font-sans text-[10px] text-gray-500 leading-relaxed">
            {t("chats.protocolDesc")}
          </p>
        </div>
      </div>
